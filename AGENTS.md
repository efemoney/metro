...
- Write comments and KDocs like you're explaining something to a colleague and use plain language. Use short plain sentences with contractions. Put one fact in each sentence. Don't chain clauses with commas, such as opening with "With a parallelism of one, reads run inline" or trailing ", at most N at a time". Don't overcorrect into terse fragments either. Keep inline comments to one sentence and skip them when the code already says it.
